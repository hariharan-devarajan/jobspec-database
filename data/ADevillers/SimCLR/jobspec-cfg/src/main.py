...
def eval_clsf(model, linear, criterion, loader, args, device, is_master, global_rank, world_size):
    ...
    with torch.no_grad():
        ...
        if args.hardware == 'multi-gpu':
            ...
            # Seems to prevent this bug:
            # https://discuss.pytorch.org/t/distributed-all-reduce-returns-strange-results/89248
            if tracker.get('epoch_clsf').count + 1 >= args.nb_epochs_clsf:
                time.sleep(1)
...
def linear_eval(args, model, train_clsf_loader, eval_clsf_loader, train_clsf_sampler, eval_clsf_sampler, scaler, global_rank, local_rank, world_size, is_master, device):
    ...
    cosine_scheduler_clsf = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer_clsf, (1+args.nb_epochs_clsf)*len(train_clsf_loader), eta_min=0.0001  # EPOCH GLITCH ?
    )
...
def run(args):
    ...
    # INIT TRACKER
    global tracker
    tracker = Tracker(device, world_size)
    ...
    ######### MODEL and CO #########
    # CREATE MODEL
    model = SimCLR(args.resnet_type, args.z_dim, args.dataset_name=='cifar10').to(device)
    if checkpoint is not None:
    ...
    cosine_scheduler_repr = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer_repr, (1+args.nb_epochs - args.nb_epochs_warmup)*len(train_repr_loader), last_epoch=last_step, eta_min=1e-8  # EPOCH GLITCH ?
    )
    if checkpoint is not None:

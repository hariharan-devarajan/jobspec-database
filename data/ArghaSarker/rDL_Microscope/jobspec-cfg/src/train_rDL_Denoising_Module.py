...
ndirs = args.ndirs
print(f'   :: RDL MODULE ::  total itter :  {total_iterations}  : sample_interval  : {sample_interval} :  validate_interval : {validate_interval} : validate_num : {validate_num} ')
...
optimizer_dn = optimizers.Adam(learning_rate=start_lr, beta_1=0.9, beta_2=0.999)
optimizer_sr = optimizers.Adam(learning_rate=start_lr, beta_1=0.9, beta_2=0.999)
...
if load_weights_flag:
    ...
    print( f'  DN module : weight file : action: weights_file[-1], wight_file there {len(weights_file)} {weights_file} ')
...
def validate(iter, sample=0):
    print('_______________        DN MODULE     :     validation and sampling   :   ________________   ')
    ...
    if sample:
        ...
        print(f'   RDL INFERENE MODULE : validate_img_files.__len__() : {validate_path.__len__()}  ')
    ...
    for path in validate_path:
        ...
        print('DN MODULE: Input: img_in : STep: val nad sampling :  DATAlOADER_RDL --------------------> ', img_in.shape)
        print('DN MODULE: Input: img_gt : STep: val nad sampling :  DATAlOADER_RDL --------------------> ', img_in.shape)
        
        img_in = np.squeeze(img_in)
        img_gt = np.squeeze(img_gt)
        print('DN MODULE: Input: img_in, img_gt : STep: val nad sampling :  DATAlOADER_RDL : np.squeeze  img_gt, img_in ,  to get rid of the 1 dimension. ')
        print('DN MODULE: Input: img_in : STep: val nad sampling :  DATAlOADER_RDL : action: np.squeeze --------------------> ', img_in.shape)
        print('DN MODULE: Input: img_gt : STep: val nad sampling :  DATAlOADER_RDL : action: np.squeeze --------------------> ', img_gt.shape)
        print()
        print('DN MODULE: Input: img_in : STep: val nad sampling :  DATAlOADER_RDL : only transpose img_in')
        print('DN MODULE: Input: img_in : STep: val nad sampling :  DATAlOADER_RDL : action: np.transpose((1, 2, 0) --------------------> ', np.array(img_in).transpose((1, 2, 0)).shape)


        cur_k0, modamp = cal_modamp(np.array(img_in).transpose((1, 2, 0)).astype(float), prol_OTF, pParam)
        ...
        img_pred = np.transpose(np.squeeze(img_pred.astype(float)), (2, 0, 1))
    ...
    if sample:  # only sample
        ...
        print("\U0001F923" + '---------->    I am inside the Sampling and Validation in DRL inferance Module : MODEL SAVE FUNCTION  <----------------------------   ')
        if min(validate_nrmse) > np.mean(nrmses):
            print('I am inside min(validate_nrmse) > np.mean(nrmses) : should save : _Best.h5   : RDL MODULE')
            #g.save_weights(save_weights_file + '_Best.h5')
            g.save_weights(save_weights_file + '_Best.weights.h5')
        else:
            print('I am inside min(validate_nrmse) > np.mean(nrmses) : should save : _Latest.h5   : RDL MODULE')
            #g.save_weights(save_weights_file + '_Latest.h5')
            g.save_weights(save_weights_file + '_Latest.weights.h5')



        print("\U0001F923" + '---------->    I am inside the Sampling and Validation in DRL inferance Module   <----------------------------   ')
...
for iter in range(total_iterations):
    print('_______________        DN MODULE     :     Model TRAINING   :   ________________   ')
    # ------------------------------------------------------------------------------
    #                     load data and calculate SIM  & phase
    # ------------------------------------------------------------------------------
    img_in, img_gt = data_loader_rDL(images_path, train_img_path, train_gt_path, batch_size)
    print('DN MODULE: Input: img_in : STep: training :  DATAlOADER_RDL --------------------> ', img_in.shape)
    print('DN MODULE: Input: img_gt : STep: training :  DATAlOADER_RDL --------------------> ', img_gt.shape)
    img_in = np.squeeze(img_in)
    img_gt = np.squeeze(img_gt)
    print('DN MODULE: Input: img_in : STep: training : action : np.squeeze: DATAlOADER_RDL --------------------> ', img_in.shape)
    print('DN MODULE: Input: img_gt : STep: training : action : np.squeeze: DATAlOADER_RDL --------------------> ', img_gt.shape)
    print('DN MODULE: Input: img_gt : STep: training : action : np.squeeze: DATAlOADER_RDL : only gt')
    print('DN MODULE: Input: img_gt : STep: training : data_transpose: action: np.transpose((1, 2, 0), only to img_gt  -------------------->  ',  np.array(img_gt).transpose((1, 2, 0)).shape)

   

    cur_k0, modamp = cal_modamp(np.array(img_gt).transpose((1, 2, 0)).astype(float), prol_OTF, pParam)
    ...
    # ------------------------------------------------------------------------------
    print(f' RDL : itter +1 : {iter + 1} : sample_interval : {sample_interval} ' , ((iter + 1) % sample_interval))
    if (iter + 1) % sample_interval == 0:
        images_path = glob.glob(train_img_path + '*')
        validate(iter + 1, sample=1)
    print(f' RDL : itter +1 : {iter + 1} : validate_interval : {validate_interval} ' , ((iter + 1) % validate_interval))
    if (iter + 1) % validate_interval == 0:
        validate(iter + 1, sample=0)

        print("\U0001F923" + '---------->    I am inside the Sampling and Validation in TRAIN LOOP in DRL inferance Module   <----------------------------   ')

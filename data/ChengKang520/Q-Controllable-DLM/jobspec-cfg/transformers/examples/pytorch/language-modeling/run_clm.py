...
class ModelArguments:
    ...
    roc_train: Optional[str] = field(
        default='/home/kangchen/Diffusion-LM-main/datasets/ROCstory',
        metadata={"help": "roc story path"},
    )
    wiki_train: Optional[str] = field(
        default='/home/kangchen/Diffusion-LM-main/datasets/simple_wiki/data.v1.split/simple.training.txt',
        metadata={"help": "simple wiki path"},
    )
    e2e_train: Optional[str] = field(
        default='/home/kangchen/Diffusion-LM-main/datasets/e2e_data',
        metadata={"help": "simple wiki path"},
    )
    ...
    threshold_ratio_inhi: Optional[float] = field(
        default=0.0,
        metadata={"help": "Choose Inhibition Level from 0 to 1."},
    )
    gi_key_side: bool = field(
        default=False,
        metadata={"help": "Inhibit KEY side."},
    )
    gi_query_side: bool = field(
        default=False,
        metadata={"help": "Inhibit QUERY side."},
    )
    gi_value_side: bool = field(
        default=False,
        metadata={"help": "Inhibit VALUE side."},
    )
    quant_m: str = field(
        default='original',
        metadata={"help": "Quantization Adaptor: 'bnn'  'ternary'  'int'  'fixed'."},
    )
    weight_i_width: int = field(
        default=8,
        metadata={"help": "Quantization Integer."},
    )
    weight_f_width: int = field(
        default=8,
        metadata={"help": "Quantization Float."},
    )

    apply_lora: bool = field(
        default=False,
        metadata={"help": "LoRA Adaptor: True or False."},
    )
    lora_r: int = field(
        default=8,
...
def get_corpus_rocstory(data_args):
    ...
    if data_args.experiment.startswith('roc') and data_args.task == 'infill':
        ...
        tokenizer2 = load_tokenizer('e2e-tgt', 'random',
                                   '/home/kangchen/Diffusion-LM-main/improved-diffusion/diffusion_models/diff_e2e-tgt_original_0_0_block_rand16_transformer_lr0.0001_0.0_2000_sqrt_Lsimple_h128_s2_d0.1_sd102_xstart_e2e/')
...
def main():
    ...
    if model_args.experiment.startswith('synth'):
        import yaml, torch
        sys.path.insert(0, '/home/kangchen/Diffusion-LM-main/improved-diffusion/diffusion_models/synthetic_data/rnns-stacks')
        ...
        dataset = dataset.load_from_disk('/home/kangchen/Diffusion-LM-main/improved-diffusion/diffusion_models/wikitext-2-pos')
        counter = Counter()
        print(dataset)
    ...
    ###################### LOAD DATASETS & dictionary #########################
        ...
        if model_args.experiment.startswith('roc'):
            tokenizer = load_tokenizer('roc', 'random',
                                       '/home/kangchen/Diffusion-LM-main/improved-diffusion/diffusion_models/diff_roc_original_0_0_pad_rand128_transformer_lr0.0001_0.0_2000_sqrt_Lsimple_h128_s2_d0.1_sd101_xstart_e2e/')
        ...
        raw_datasets = train_datasets.train_test_split(0.1)
        print(raw_datasets)
        ...
        elif model_args.experiment in ['e2e-tgt-tree', 'e2e-tgt-gen-tree', 'e2e-tgt-spans', 'e2e-tgt-gen-spans']:
            import benepar
    ...
    config_kwargs = {
        ...
        "threshold_ratio_inhi": model_args.threshold_ratio_inhi,
        "gi_key_side": model_args.gi_key_side,
        "gi_query_side": model_args.gi_query_side,
        "gi_value_side": model_args.gi_value_side,
        "quant_m": model_args.quant_m,
        "weight_i_width": model_args.weight_i_width,
        "weight_f_width": model_args.weight_f_width,
    ...
    if model_args.model_name_or_path:
        ...
        ############# LOAD MODELS for controllable classifier ##############
        elif model_args.experiment in ['e2e-back', 'e2e-back_t2', 'e2e-tgt-pos', 'e2e-tgt-tree', 'e2e-tgt-spans']:
            import torch
            config.vocab_size = len(tokenizer)
            print('\n Initializing the model from scratch \n' + '*' * 100)
            ...
            config.gi_key_side = model_args.gi_key_side
            config.gi_query_side = model_args.gi_query_side
            config.gi_value_side = model_args.gi_value_side
            config.quant_m = model_args.quant_m
            config.weight_i_width = model_args.weight_i_width
            config.weight_f_width = model_args.weight_f_width
            ...
            elif model_args.experiment == 'e2e-tgt-spans':
                config.tree_vocab_size = len(tree_vocab)
                print('tree vocab size', len(tree_vocab), '*'*20)
                model = Classifier_Tree(config=config, diffusion=diffusion)
            ...
            elif model_args.experiment == 'e2e-tgt-spans' and model_args.learned_emb == 'no':
                model.transformer.embeddings.word_embeddings.load_state_dict(torch.load(path_save))
                model.transformer.embeddings.word_embeddings.weight.requires_grad = False
            elif model_args.experiment == 'e2e-tgt-spans' and model_args.learned_emb == 'yes':
                print('loading the learned embeddings')
                learned_embeddings = torch.load(path_learned)['word_embedding.weight']
                model.transformer.embeddings.word_embeddings.weight.data = learned_embeddings.clone()
                model.transformer.embeddings.word_embeddings.weight.requires_grad = False
    ...
    if model_args.experiment.startswith('synth'):
        ...
        def tokenize_function(examples):
            ...
            with CaptureLogger(tok_logger) as cl:
                ...
                if model_args.experiment == 'e2e-tgt-pos':
                    ...
                    if model_args.task == 'finetune':
                        input_strings = [" ".join(pos_) + " ".join(seq)
        ...
        def tokenize_function(examples):
            ...
            with CaptureLogger(tok_logger) as cl:
                if model_args.task == 'finetune':
                    ...
                    # return tokenizer(input_strings, max_length=128, padding='max_length', truncation=True)
                    input_strings = [f'{len(seq)}' + " ".join(seq)
                                     for seq in examples['text']]
                    return tokenizer(input_strings, max_length=128, padding='max_length', truncation=True)
                elif model_args.task == 'from_scratch':
        ...
        from utils import chart_from_tree, remove_leaves, pad_charts, padded_chart_from_spans
        ...
        def tokenize_function(examples):
            ...
            with CaptureLogger(tok_logger) as cl:
                ...
                if model_args.experiment == 'e2e-tgt-tree':
                    chart_lst = []
                    for x in parse_lst:
                        chart, spans = chart_from_tree(tree_vocab, x)
                ...
                # if model_args.experiment == 'e2e-tgt-spans':
                #     chart_lst = []
                #     spans_lst = []
                #     input_lst = []
                #     for parse, seq in zip(parse_lst, examples['text']):
                #         chart, spans = chart_from_tree(tree_vocab, parse)
                #         spans_lst.append(chart)
                #     input_ids = [[0] + [vocab_dict.get(x, vocab_dict['UNK']) for x in seq] + [1] for seq in examples['text']]
                #     result_dict = {'input_ids': input_ids, 'spans_lst': spans_lst}

                if model_args.experiment == 'e2e-tgt-spans':
                    chart_lst = []
                    for parse, seq in zip(parse_lst, examples['text']):
                        chart = padded_chart_from_spans(tree_vocab, parse)
                        chart_lst.append(chart)
                        # for (a, b, c) in spans:
                    ...
                    input_ids = [[0] + [vocab_dict.get(x, vocab_dict['UNK']) for x in seq] + [1] for seq in examples['text']]
                        ...
                        #              examples['text']]
                        # for (a, b, c) in spans:
                        #     spans_i = [vocab_dict.get(x, vocab_dict['UNK']) for x in f"{a} {b} {c}".split()]
                        #     print('#############    spans_i    ###############')
                        #     print(spans_i)
                        #     print('#############    spans_i    ###############')
                        #     chart_lst.append(spans_i)
                    result_dict = {'input_ids': input_ids, 'chart_lst': chart_lst}
                    ...
                    if model_args.task == 'finetune':
                        ...
                        for parse, seq in zip(parse_lst, examples['text']):
                            ...
                            for (a,b,c) in spans:
                                input_strings.append(f"{a}, {b}, {c}" + " ".join(seq))
    ...
    trainer_tokenizer = None if ((model_args.experiment in ['pos', 'synth', 'roc', 'simple-wiki', 'e2e-tgt',
                                                            'e2e-tgt-pos','e2e-tgt-tree','e2e-tgt-spans', 'e2e-back', 'e2e-back_t2']

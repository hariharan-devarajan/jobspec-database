...
# module to be used for input files in fastq format. This module schedules fastqc, alignment and dedup jobs
...
if [ ! -d $fastqcdir -a $fastqcflag == "YES" ]
then
...
truncate -s 0 $TopOutputLogs/*.joblist
truncate -s 0 $TopOutputLogs/align/*.joblist 
	...
	then
		...
		do
			...
			then
				...
				then
					set +x; echo -e "################# CASE is NOT TO SPLIT ANALYSIS, alignment and markduplicates in one job ############\n" >&2; set -x
					...
					set +x; echo -e "################# CASE is SPLIT ANALYSIS, alignment in one job and markduplicates in another job ##\n" >&2; set -x
...
set +x; echo -e "\n\n\n" >&2
echo "#####################################################################################################################" >&2
echo "##########  SCHEDULE ALIGNMENT JOBS                                               ###################################" >&2
echo "##########  CASE1: NO CHUNKING OF INPUT FILES, BWA_MEM, LAUNCHER                  ###################################" >&2
echo "##########  CASE2: NO CHUNKING OF INPUT FILES, BWA_MEM, NON-LAUNCHER              ###################################" >&2
echo "##########  CASE3: CHUNKING OF INPUT FILES, NOVOSORT, LAUNCHER                    ###################################" >&2
echo "#####################################################################################################################" >&2
echo -e "\n\n" >&2; set -x
...
if [ $chunkfastq == "NO" -a $aligner == "BWA_MEM" -a $paired -eq 1 -a $run_method == "LAUNCHER" ]
then
...
elif [ $chunkfastq == "NO" -a $aligner == "BWA_MEM" -a $paired -eq 1 -a $run_method != "LAUNCHER" ]
then
...
elif [ $chunkfastq == "YES" -a $aligner == "NOVOALIGN" -a $sortool == "NOVOSORT" ]
then

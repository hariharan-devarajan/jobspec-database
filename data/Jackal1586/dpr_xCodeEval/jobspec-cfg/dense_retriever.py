	...
	with torch.no_grad():
		for j, batch_start in enumerate(range(0, n, bsz)):
			...
			if selector:
				...
				# remove q_seg_batch from q_ids_batch, q_seg_batch, q_attn_mask
				out = question_encoder(q_ids_batch, q_attn_mask)
...
def main(cfg: DictConfig):
	...
	model_config = AutoConfig.from_pretrained(cfg.encoder.pretrained_model_cfg)
	# vector_size = model_to_load.get_out_size() #self.config.hidden_size

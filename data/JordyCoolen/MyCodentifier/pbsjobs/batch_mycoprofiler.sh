    ...
    base=$(basename "$f" .fastq.gz)
        if [[ $f == *"_R"* ]]; then
            ...
            if [[ ! " ${processed[@]} " =~ " ${reads} " ]]; then
                ...
                { time singularity exec -B ${reads_dir}:/data,/ifs/software/external/mmb/mycoprofiler:/workflow nextflow run /workflow/myco.nf --threads 16 --reads \"$reads\" --sampleName \"$name\" --outDir \"$out_dir\" --subsampling true --snpeff_base snpEff/ 1 >> ${out_dir}/${name}/log.stderr ; } 2> ${out_dir}/${name}/time.txt &&\n\
                out_dir=${out_dir} &&\n\
                name=${name} &&\n\

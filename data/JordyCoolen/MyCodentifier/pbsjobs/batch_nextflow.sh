...
# path of nextflow script
myco_nf="/home/gerwin/hseverin/project/nextflow_proj/myco.nf"
...
do
    base=$(basename "$f" .fastq.gz)
		...
		(git log -1 && echo -e "\n") > ${out_dir}/${name}/log.stderr
                ...
                { time nextflow run ${myco_nf} --threads 16 --reads \"$reads\" --sampleName \"$name\" --outDir \"$out_dir\" --subsampling true 1>> ${out_dir}/${name}/log.stderr ; } 2> ${out_dir}/${name}/time.txt &&\n\
                out_dir=${out_dir} &&\n\
                name=${name} &&\n\

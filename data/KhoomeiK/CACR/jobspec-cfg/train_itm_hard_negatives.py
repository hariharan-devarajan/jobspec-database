...
def main(opts):
    ...
    #     ct += 1
    #     if ct >= 256: # num of params to freeze
    #         break
    ...
    LOGGER.info("  Num examples = %d",
                sum(all_gather_list(len(train_dataset_t))) if n_gpu else len(train_dataset_t))
    ...
    while True:
        # @profile(stream=fp)
        def iteration(): # for batch in train_dataloader_t:
            nonlocal step, n_examples, n_hard_ex, train_iter_i, global_step
            ...
            if opts.IAIS:
                ...
                self_attn_tsa_loss = self_attn_loss_per_layer['self_attn_loss'] * get_tsa_threshold(opts.tsa_schedule,
                                                                                                    global_step,
            ...
            if step % opts.train_batch_size == 0:
                ...
                del batch_i, rank_loss, self_attn_loss_per_layer, loss
...
if __name__ == "__main__":
    ...
    parser.add_argument('--IAIS', default=False, choices=['distributed', 'singular', 'soft', False],
                        help='msa regularizer')
    parser.add_argument('--tsa_schedule', default='exp_schedule', choices=['linear_schedule', 'exp_schedule', 'log_schedule', 'const_schedule'],
                        help='training signal annealing schedule')

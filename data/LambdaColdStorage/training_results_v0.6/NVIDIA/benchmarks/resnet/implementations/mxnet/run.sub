...
CLEAR_CACHES=1
SYSLOGGING=1
LOG_CLEAR_CACHES="'from mlperf_compliance import constants, mlperf_log as log; import os; log.mlperf_print(key=constants.CACHE_CLEAR, value=True, benchmark=constants.${BENCHMARK_NAME}, stack_offset=0, root_dir=\"`pwd`\", extra_print=False)'"
...
if [[ ! -f "config_${DGXSYSTEM}.sh" ]]; then
  echo "ERROR: Unknown system config ${DGXSYSTEM}"
  exit 1
fi
...
if [[ ! $KVSTORE =~ "horovod" ]]; then
  ...
  [[ "$DEBUG" != "0" ]] && TAGOUTPUT="--tag-output"
  MPIRUN=( mpirun --allow-run-as-root --bind-to none $TAGOUTPUT ${VARS[@]//-e/-x} --launch-agent "docker exec $CONTNAME orted" )
fi
...
$SRUNl nvidia-docker run --init -d "${DOCKERRUN_ARGS[@]}" --name $CONTNAME $VOLS "${VARS[@]}" $CONT bash -c 'rm -f /etc/shinit && sleep infinity' ; rv=$?
...
if [[ $SLURM_JOB_NUM_NODES -gt 1 ]] || [[ $DGXSYSTEM =~ "_multi_" ]]; then

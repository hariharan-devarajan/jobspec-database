...
module purge
module load gromacs/2021.2/gcc.8.4.0-cuda.10.1.243

gmx grompp -f npt.mdp -c ../../NPT/npt.gro -p ../../complex.top -o npt.tpr -maxwarn 1

cpi_file="npt.cpt"
if test -f "$cpi_file"; then
	echo RUNNING NPT CONTINUATION: cpt file exists
	gmx mdrun -deffnm npt -ntomp 12 -ntmpi 1 -s npt.tpr -cpi npt.cpt -append
else
	echo RUNNING NPT FROM BEGINNING: cpt file doesnt exist
	gmx mdrun -deffnm npt -ntomp 12 -ntmpi 1 -s npt.tpr
fi
echo Job Done
}

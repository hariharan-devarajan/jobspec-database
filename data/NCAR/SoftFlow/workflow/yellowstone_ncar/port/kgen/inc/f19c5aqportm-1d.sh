...
if [ "$u" = "1" ]; then
    ...
    $cfgdir/configure $compiler -dyn fv -hgrid 1.9x2.5 -phys cam5 -ocn aquaplanet -offline_drv rad \
...
if [ -z "$b" ]; then
	...
	-namelist "&atm_in start_type='startup' start_ymd=101
         ...
         iradsw=1 iradlw=1 ndens=2,1 nhtfrq=0,1 mfilt=1,49
    ...
    echo "control run in $rundir using $ntasks tasks and no threading"

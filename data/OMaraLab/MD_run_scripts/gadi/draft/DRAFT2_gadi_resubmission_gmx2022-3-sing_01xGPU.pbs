...

{
    ...
    if [ $errstat != 0 ]; then
        ...
        sleep 5
        echo "Job returned error status $errstat - stopping job sequence $PBS_JOB_NAME at job $PBS_JOB_ID"
        exit $errstat
    fi
}
...
if [ ! -f ${PBS_JOB_NAME}.tpr ]; then
    $GMX grompp -f ${PBS_JOB_NAME}.mdp -c ${PBS_JOB_NAME}_start.gro -o ${PBS_JOB_NAME}.tpr -p ${PBS_JOB_NAME}.top  -n ${PBS_JOB_NAME}.ndx -maxwarn 2 &> ${PBS_JOB_NAME}_grompp_${PBS_JOB_ID}.txt
fi

#####  RUN MD FOR 9.95 HOURS  #####

$GMXMDRUN -v -deffnm ${PBS_JOB_NAME} -cpi ${PBS_JOB_NAME}.cpt || errexit

#####  CHECK IF JOB IS DONE; IF NOT DONE RESUBMIT THIS SCRIPT  #####


# Check the log file for the number of steps completed
steps_done=`perl -n -e'/Statistics over (\d+) steps using (\d+) frames/ && print $1' ${PBS_JOB_NAME}.log`
# Check the mdp file for the number of steps we want
steps_wanted=`perl -n -e'/nsteps\s*=\s*(\d+)/ && print $1' ${PBS_JOB_NAME}.mdp`
# Resubmit if we need to
if (( steps_done < steps_wanted )); then
    echo "Job ${PBS_JOB_NAME} terminated with ${PBS_JOB_NAME}/${PBS_JOB_NAME} steps finished." 
    echo "Submitting next job in sequence ${PBS_JOB_NAME}."
    qsub ${PBS_JOB_NAME}
fi

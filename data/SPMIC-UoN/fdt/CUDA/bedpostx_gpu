...
Usage() {
    ...
    echo "-Q (name of the GPU(s) queue, default --coprocessor=cuda to let fsl_sub decide on the queue"
...
if [ "$qsys" -eq 0 ] && [ -z "$queue" ]; then
    if "$FSLDIR/bin/fsl_sub" --has_coprocessor='cuda' > /dev/null; then
        queue="--coprocessor=cuda"
    else
        echo "No GPU configured/found" >&2
        exit 1
    fi
fi
...
do
    ...
    xf_gpu=(\"${FSLDIR}/bin/xfibres_gpu\" --data=\"${subjdir}.bedpostX/data_$part\" --mask=\"$subjdir.bedpostX/nodif_brain_mask\" \
        -b \"${subjdir}.bedpostX/bvals\" -r \"${subjdir}.bedpostX/bvecs\" --forcedir --logdir=\"$subjdir.bedpostX/diff_parts/data_part_$partzp\" \
        "${gopts[@]}" "${subjdir}" "$part" "$njobs" "$nvox")
    echo "${xf_gpu[@]}" >> "${subjdir}.bedpostX/commands.txt"
...
post_command=("${FSLDIR}/bin/bedpostx_postproc_gpu.sh" "--data=${subjdir}/data" "--mask=$subjdir.bedpostX/nodif_brain_mask" \
    -b "${subjdir}.bedpostX/bvals" -r "${subjdir}.bedpostX/bvecs" --forcedir "--logdir=$subjdir.bedpostX/diff_parts" "${gopts[@]}" \

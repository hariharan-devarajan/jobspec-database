...
def load_group(dataset, group_tree=0):
    ...
    if dataset == 'amazontitles300k':
        return np.load(f'/users/kabrasan/multimodalxmc/data/AmazonTitles-300K/label_group{group_tree}.npy', allow_pickle=True)
...
def train(model, df, label_map):
    ...
    log_str = f'   Training model for the congiguration {vars(args)} '
    LOG.log(log_str)
    ...
    if args.dataset in ['wiki500k', 'amazon670k','amazontitles300k']:
        ...
        trainloader = DataLoader(train_d, batch_size=args.batch, num_workers=10,
                                 shuffle=True)
        testloader = DataLoader(test_d, batch_size=args.batch, num_workers=10,
                                shuffle=False)
        args.valid = True
        if args.valid:
            valid_d = MDataset(df, 'valid', tokenizer, label_map, args.max_len, group_y=group_y,
                               candidates_num=args.group_y_candidate_num)
            validloader = DataLoader(valid_d, batch_size=args.batch, num_workers=10, 
                                     shuffle=False)
    else:
    ...
    for epoch in range(0, args.epoch+5):
        ...
        LOG.log(log_str)
        if args.use_wandb:
            wandb_log = {'Test/P@1':p1,'Test/P@3':p3,'Test/P@5':p5,'Test/g_P@1':g_p1,'Test/g_P@3':g_p3,'Test/g_P@5':g_p5,'Train/Loss':train_loss}
            wandb.log(wandb_log)
...
parser.add_argument('--batch', type=int, required=False, default=128)
...
parser.add_argument('--dataset', type=str, required=False, default='amazontitles300k')
...
parser.add_argument('--hidden_dim', type=int, required=False, default=400)

parser.add_argument('--eval_model', action='store_true')
parser.add_argument('--use-wandb', default=False)
parser.add_argument('--run-name', default='Amazon670')
...
if __name__ == '__main__':
    ...
    if args.use_wandb:
        ...
        wandb.init(project="Multimodal-XMC",config=vars(args),name=args.run_name)

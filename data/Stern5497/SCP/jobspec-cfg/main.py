...
task_code_mapping = {
    ...
    'mapa_coarse': 'NER',
    'mapa_fine': 'NER',
    'swiss_bge_criticality_prediction': 'SLTC',
}
...
max_sequence_lengths = {  # 256, 512, 1024, 2048, 4096
    ...
    'swiss_bge_criticality_prediction': 16 * 128  # TODO get the right max sequence length for bge prediction
}
...
def generate_command(time_now, **data):
    ...
    with open('temporary_scripts/commands.txt', 'w') as f:
        print(final_command, file=f, end='\n')
...
def get_optimal_batch_size(language_model: str, task: str, gpu_memory, total_batch_size=64):
    ...
    logger.info(f"language model: {language_model} and task {task} and gpu_memory: {gpu_memory}")
    try:
        batch_size_dict = optimal_batch_sizes[int(gpu_memory)][str(language_model)]
    except:
        print("The language model ", language_model,
              "will be considered a monolingual model. Therefore, we revert to the default batch size.")

'Calculating boots from for DX cohens D
...
combat_data_corZ <- readRDS(file = file.path(output_base, "all_clinicalplusqa_group", "Rdata_cache", "06_wholebrain_FC_PINTcombat_cache.rds"))
...
outfile <- file.path(output_base, 
          ...
          "Rdata_cache", 
          "06_PINTFC_boots", 
          str_c("06_wholebrain_FC_PINTcombat_roi_DXboots_cache",
                ibasef,
                ".rds"))
...
for (i in ibase:(ibase + 9)) {
  ...
  #for (this_to in unique(results_pheno$to)) {
    ...
    this_result <-  results_pheno %>%
      ...
      mutate("pvertex_minus_tvertex" = pvertex - tvertex,
             "tvertex_minus_tvolume" = tvertex - tvolume,
             "pvertex_minus_tvolume" = pvertex - tvolume,
              i = i)
  #}          

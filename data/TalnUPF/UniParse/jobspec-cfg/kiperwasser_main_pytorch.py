...
def do_training_big_datasets(arguments, vocab, subset_size):
...
def main():
    ...
    vocab = load_or_create_vocab(arguments)
    ...
    if arguments.do_training:
        if not arguments.big_dataset:
            ...
            parser = do_training(arguments, vocab)
        else:
            subset_size = 10000
            logging.info('Training with big dataset; subset_size = %i' % subset_size)
            parser = do_training_big_datasets(arguments, vocab, subset_size)

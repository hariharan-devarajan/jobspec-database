...
def init_nets(net_configs, dropout_p, n_parties, args):
    ...
    if args.use_projection_head:
        ...
        for net_i in range(n_parties):
            print('lalalalalalalalalalhahahahahahahaha')
...
def train_net_fednova(net_id, net, global_model, train_dataloader, test_dataloader, epochs, lr, args_optimizer,
    ...
    logger.info(' ** Training complete **')
    return train_acc, test_acc, a_i, norm_grad, avg_loss
...
def local_train_net_fednova(nets, selected, global_model, args, net_dataidx_map, test_dl=None, device="cpu"):
    ...
    for net_id, net in nets.items():
        ...
        trainacc, testacc, a_i, d_i, avg_loss = train_net_fednova(net_id, net, global_model, train_dl_local, test_dl, n_epoch,
    ...
    nets_list = list(nets.values())
    return nets_list, a_list, d_list, n_list, avg_loss
...
if __name__ == '__main__':
    ...
    # for party_id in range(args.n_parties):
    #     dataidxs = net_dataidx_map[party_id]
    #     print("party_id hhhhhhhhhh:", party_id)
    #
    #     train_dl_local, test_dl_local, train_ds_local, test_ds_local = get_dataloader(args.dataset,
    #                                                                                       args.datadir,
    #                                                                                       args.batch_size,
    ...
    if args.alg == 'fedavg':
        ...
        for round in range(args.comm_round):
            ...
            _, a_list, d_list, n_list, avg_loss = local_train_net_fednova(nets, selected, global_model, args, net_dataidx_map,
                                                                test_dl=test_dl_global, device=device)
            total_n = sum(n_list)
    ...
    logger.info("All Test Accuracy Data: " + str(y_acc))
    logger.info("All Loss Data: " + str(y_loss))
    plt.figure(figsize=(10,10))
    plt.plot(x, y_acc, label="x", color="#3399FF", marker='.', linestyle="-")
    plt.gca().xaxis.set_major_locator(ticker.MultipleLocator(1))
    plt.xlabel("comm round")
    plt.ylabel("test acc")
    plt.savefig("./myfig/cifar10_random_0.05_acc_4.png")

    plt.figure(figsize=(10,10))
    plt.plot(x, y_loss, label="x", color="#3399FF", marker='.', linestyle="-")
    plt.gca().xaxis.set_major_locator(ticker.MultipleLocator(1))
    plt.xlabel("comm round")
    plt.ylabel("test loss")
    plt.savefig("./myfig/cifar10_random_0.05_loss_4.png")

            ...
            for layer in FLAGS.display:
                if 'latlong' in layer and layer['latlong']:
                    ...
                    img = render.render_mesh(FLAGS, glctx, opt_mesh, target['mvp'], target['campos'], target['light'] if lgt is None else lgt, target['resolution'],
                                                spp=target['spp'], num_layers=FLAGS.layers, background=white_bg, bsdf=layer['bsdf'], optix_ctx=geometry.optix_ctx,
                                                 )['shaded']
                    if layer['bsdf'] == 'kd':
...
if __name__ == "__main__":
    ...
    # read bbox
    o3d_mesh = o3d.t.io.read_triangle_mesh(FLAGS.base_mesh)
    FLAGS.centre = o3d_mesh.get_center().numpy()
    FLAGS.mesh_scale = (o3d_mesh.get_max_bound() - o3d_mesh.get_min_bound()).max().item() + 0.1
    print(f"Using mesh scale : {FLAGS.mesh_scale}, center : {FLAGS.centre}")
    ...
    if os.path.splitext(FLAGS.ref_mesh)[1] == '.obj':
        ...
        if os.path.isfile(os.path.join(FLAGS.ref_mesh, 'poses_bounds.npy')):
            ...
            dataset_validate = DatasetLLFF(FLAGS.ref_mesh, FLAGS, validate=True, center=dataset_train.center)
        ...
        # elif os.path.isfile(os.path.join(FLAGS.ref_mesh, 'cameras_sphere.npz')):
        #     dataset_train    = DatasetLLFF(FLAGS.ref_mesh, FLAGS, examples=(FLAGS.iter+1)*FLAGS.batch)
        #     dataset_validate = DatasetLLFF(FLAGS.ref_mesh, FLAGS, validate=True)
        else:

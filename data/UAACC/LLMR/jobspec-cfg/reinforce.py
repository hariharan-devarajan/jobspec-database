...
def reward_fn(reward_model, input_ids, output_ids, args, pad_token_id=None):
    with torch.no_grad():
        ...
        # logits = logits - torch.sum(torch.softmax(logits, dim=-1) * logits, dim=-1, keepdim=True)
        ...
        return selection_value - next_state_value
...
def actor_proc(queue, model, reward_model, handler, dataset, args, world_size, local_rank, device, free_to_go):
    ...
    with torch.no_grad(), amp_context:
        while True:
            ...
            for _ in range(epoch_length):
                ...
                src_sents = [tokenizer.decode(sent_ids, skip_special_tokens=True) for sent_ids in enc_input]
                prompt_inputs = handler.process(src_sents)
                ...
                for l in range(max_length):
                    ...
                    if args.mask_extra:
                        logits[..., 32000:] = -torch.inf#for <extra_id_0> generated in beginning
                ...
                for t in range(len(prompt_inputs)):
                    prompt_ids = tokenizer(prompt_inputs[t], return_tensors='pt', padding=True).to(device)
                    prompt_ids = prompt_ids['input_ids'].index_select(0, ready_index)
                    # print(prompt_ids.shape, input_ids.shape, flush=True)
                ...
                queue.put((enc_input, input_ids, b_probs, ready_mask, rewards))
                del data
            del dataiter
...
class Trainer:
    def __init__(
        ...
        self.template_handler = TemplateHandler(args.template, self.tokenizer)
        
        for i in range(num_actor):
            self.actor_procs.append(
                # queue, model, reward_model, handler, dataset, args, world_size, local_rank, device, free_to_go
                mp.Process(
                    target=actor_proc,
                    args=(self.queue, self.gen_model, self.reward_model, self.template_handler, self.dataset, args, args.world_size * num_actor, self.rank * num_actor + i, device, free_to_go)))
    ...
    def train(self):
        ...
        while self.update_steps < self.num_training_steps:
            for in_epoch_step in range(self.epoch_length):
                ...
                enc_input, input_ids, b_probs, ready_mask, rewards = data
                ...
                with ddp_context:
                    ...
                    # print(max(p.grad.max().item() for p in self.model.parameters()))
                    ...
                    if dist.get_rank() == 0:
                        try:
                            if self.update_steps % self.update_per_log == 0:
                                for t, r, lp in zip(input_ids[0], rewards[0], lprobs[0]):
                                    print(f'{self.tokenizer.decode(t.item())}({r.item():.4g}:{lp.item():.4g})', end=' ')
                                print(min(int(1.5 * enc_input.shape[-1]), self.args.max_length), flush=True)
                                self.log({
...
def distributed_main(rank, args, model, dataset, tokenizer):
    ...
    if tokenizer.pad_token_id == model.config.decoder_start_token_id:
        setattr(tokenizer, 'pad_token_id', tokenizer.vocab['<extra_id_1>'])
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--cumprod', action='store_true')
    ...
    distributed_procs = []
    # distributed_main(0, args, model, dataset, tokenizer)

...
tar -xf ${SW_SOURCES}/${PACKAGE}/testcases/apoa1.tar.gz 
mv apoa1/* .
cat > namd.pbs << EOF
#!/bin/bash
#PBS -N namd.apoa1
...
aprun -n 4 \$NAMD_BIN apoa1.namd 2>&1 | tee -a ${SW_BLDDIR}/.running > apoa1.namd.out
...
grep Memory    apoa1.namd.out | awk '{print "YVALUE=",\$6}' \
...
#   "stdbuf -oL" to remove buffering
stdbuf -oL qsub namd.pbs > ${SW_BLDDIR}/.running

...
# project imports
import ifcb
from neuston_models import MetaModel
from neuston_callbacks import SaveValidationResults, SaveTestResults
from neuston_data import get_trainval_datasets, HerringrunnerDataset, HerringRUNnerDataset
...
def do_training(args):
    ...
    if not args.result_files:
        args.result_files = ['results.json training_image_basenames training_classes image_basenames input_classes output_scores confusion_matrix counts_perclass f1_perclass f1_weighted f1_macro'.split()]
...
def do_run(args):
    ...
    # load model
    classifier = MetaModel.load_from_checkpoint(args.MODEL)
    ...
    if 'resize' not in classifier.hparams:
        resize_val = 299 if classifier.hparams.MODEL=='inception_v3' else 224
    else:
        resize_val = classifier.hparams.resize

    image_dataset = HerringRUNnerDataset(img_paths, resize=resize_val, input_src=args.SRC)
...
def argparse_nn_train(train_subparser):
    ...
    train_subparser.add_argument('TRAINING', help='Directory with class-label subfolders and images. May also be a dataset-configuration csv.')
    train_subparser.add_argument('VALIDATION', help='Directory with class-label subfolders and images. May also be a dataset-configuration csv.')
    ...
    model.add_argument('--counts-mode', action='store_true', help='if invoked, the model returns a single count value instead of a count-class')
    ...
    #TODO data.add_argument('--categories', help='list of values are used to demarcate different categories, less-than-or-equal-to. eg 0 5 10 15 20')
...
def argparse_nn_run(run_subparser):
    ## Run Vars ##
    run_subparser.add_argument('SRC', help='Resource(s) to be classified. Accepts an image, a text-file, or a directory. Directories are accessed recursively')
...
def proc_outdir(args):
    ...
    if args.cmd_mode=='TRAIN':
        ...
        model_id = MetaModel.load_from_checkpoint(args.MODEL).hparams.model_id
...
# TODO move dataloaders to MetaModel for auto-batch-size enabling

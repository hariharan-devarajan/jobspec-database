...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.warmup_pct is None:
        ...
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=math.floor(args.warmup_pct*t_total), num_training_steps=t_total)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, eval_split="train"):
    ...
    if os.path.exists(cached_features_file):
        ...
        if eval_split == "train":
            examples = processor.get_train_examples(args.data_dir, args.data_size)
        elif eval_split == "dev":
        ...
        features = convert_qa_examples_to_partial_scoring_features(
            ...
            pad_token=tokenizer.pad_token_id, #arbitary value since its gonna get ignored anyway
...
def main():
    ...
    ## Other parameters
    parser.add_argument("--data_size", default='xs', type=str, help="Choose between xs, s, m, l")

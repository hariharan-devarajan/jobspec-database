...
if [ $peak_mode -eq 1 ]; then
    if [ -f "${sample_info[2]}" ]; then
        ...
        outdir_L25="${PBS_O_WORKDIR}/ref_genome/biorep_merged/results/loops/fithichip/${sample_name}_hichip-peaks.peaks/L25/"
        mkdir -p $outdir_L25
    else
...
if [ $peak_mode -eq 2 ]; then
    if [ -f "${sample_info[3]}" ]; then
        ...
        # make the output directory
        outdir_L25="${PBS_O_WORKDIR}/ref_genome/biorep_merged/results/loops/fithichip/${sample_name}_fithichip.peaks/L25/"
        mkdir -p $outdir_L25
    else
...
if [ $peak_mode -eq 3 ]; then
    if [ -f "${sample_info[4]}" ]; then
        ...
        # make the output directory
        outdir_L25="${PBS_O_WORKDIR}/ref_genome/biorep_merged/results/loops/fithichip/${sample_name}_chipseq.peaks/L25/"
        mkdir -p $outdir_L25
    else

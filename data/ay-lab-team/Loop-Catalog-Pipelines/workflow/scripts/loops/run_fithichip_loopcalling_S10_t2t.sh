...
if [ $peak_mode -eq 1 ]; then
    if [ -f "${sample_info[2]}" ]; then
        ...
        outdir_S10="${PBS_O_WORKDIR}/ref_genome/biorep_merged/results/loops/fithichip/${sample_name}_hichip-peaks.peaks/S10/"
        mkdir -p $outdir_S10
    else
...
if [ $peak_mode -eq 2 ]; then
    if [ -f "${sample_info[3]}" ]; then
        ...
        # make the output directory
        outdir_S10="${PBS_O_WORKDIR}/ref_genome/biorep_merged/results/loops/fithichip/${sample_name}_fithichip.peaks/S10/"
        mkdir -p $outdir_S10
    else
...
if [ $peak_mode -eq 3 ]; then
    if [ -f "${sample_info[4]}" ]; then
        ...
        # make the output directory
        outdir_S10="${PBS_O_WORKDIR}/ref_genome/biorep_merged/results/loops/fithichip/${sample_name}_chipseq.peaks/S10/"
        mkdir -p $outdir_S10
    else

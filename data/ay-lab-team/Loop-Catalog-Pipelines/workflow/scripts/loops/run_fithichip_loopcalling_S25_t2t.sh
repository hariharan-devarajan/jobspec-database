...
if [ $peak_mode -eq 1 ]; then
    if [ -f "${sample_info[2]}" ]; then
        ...
        outdir_S25="${PBS_O_WORKDIR}/ref_genome/biorep_merged/results/loops/fithichip/${sample_name}_hichip-peaks.peaks/S25/"
        mkdir -p $outdir_S25
    else
...
if [ $peak_mode -eq 2 ]; then
    if [ -f "${sample_info[3]}" ]; then
        ...
        # make the output directory
        outdir_S25="${PBS_O_WORKDIR}/ref_genome/biorep_merged/results/loops/fithichip/${sample_name}_fithichip.peaks/S25/"
        mkdir -p $outdir_S25
    else
...
if [ $peak_mode -eq 3 ]; then
    if [ -f "${sample_info[4]}" ]; then
        ...
        # make the output directory
        outdir_S25="${PBS_O_WORKDIR}/ref_genome/biorep_merged/results/loops/fithichip/${sample_name}_chipseq.peaks/S25/"
        mkdir -p $outdir_S25
    else

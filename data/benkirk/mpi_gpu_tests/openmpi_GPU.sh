...
for tool in $(find ${TESTS_DIR} -type f -executable -name osu_bw -o -name osu_bibw -o -name osu_latency | sort); do
    ...
    echo ${tool} && mpiexec -n 2 -npernode 2 $(which get_local_rank) ${tool} || status="FAIL"

    echo && echo && echo "********* Inter-Node-CPU *****************"
    echo ${tool} && mpiexec -n 2 -npernode 1 $(which get_local_rank) ${tool} || status="FAIL"

    echo && echo && echo "********* Intra-Node-GPU *****************"
    echo ${tool} && mpiexec -n 2 -npernode 2 $(which get_local_rank) ${tool} D D || status="FAIL"

    echo && echo && echo "********* Inter-Node-GPU *****************"
    echo ${tool} && mpiexec -n 2 -npernode 1 $(which get_local_rank) ${tool} D D || status="FAIL"
done
...
for tool in $(find ${TESTS_DIR} -type f -executable -name "osu_alltoall*" | sort); do
    ...
    echo && echo && echo "********* Intra-Node-CPU *****************"
    echo ${tool} && mpiexec -n 4 -npernode 4 $(which get_local_rank) ${tool} || status="FAIL"

    echo && echo && echo "********* Inter-Node-CPU *****************"
    echo ${tool} && mpiexec -n 8 -npernode 4 $(which get_local_rank) ${tool} || status="FAIL"

    echo && echo && echo "********* Intra-Node-GPU  (nranks == ngpus) *****************"
    echo ${tool} && mpiexec -n 4 -npernode 4 $(which get_local_rank) ${tool} -d managed || status="FAIL"

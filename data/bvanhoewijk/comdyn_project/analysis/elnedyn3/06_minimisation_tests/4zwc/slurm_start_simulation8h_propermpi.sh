...
if [ -f "rep${rep}/step7_production_rep${rep}.cpt" ]; then
    ...
    mpirun -np 32 gmx_mpi mdrun -cpi rep${rep}/step7_production_rep${rep}.cpt -deffnm rep${rep}/step7_production_rep${rep} -append -g rep${rep}/rep${rep}_md.log -ntomp 1
else
    # If no checkpoint:
    echo "####################### No checkpoint. Starting fresh run"
    mpirun -np 32 gmx_mpi mdrun -deffnm rep${rep}/step7_production_rep${rep} -cpt 1 -pin on -g rep${rep}/rep${rep}_md.log -maxh 8 -ntomp 1
fi

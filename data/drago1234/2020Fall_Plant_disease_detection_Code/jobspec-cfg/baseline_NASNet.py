...
metrics = [tf.keras.metrics.Precision(name='precision'), tf.keras.metrics.CategoricalAccuracy(name='accuracy')] # 'accuracy', 'precision', 'recall', 'auc', and more(https://www.tensorflow.org/tfx/model_analysis/metrics, https://www.tensorflow.org/api_docs/python/tf/keras/Model#compile)
...
last_output = NASNetLarge_model.output
x = tf.keras.layers.Flatten()(last_output)
x = tf.keras.layers.Dense(1024, activation='relu')(x)
output_layer = tf.keras.layers.Dense(no_classes, activation='softmax')(x)
model = tf.keras.Model(inputs=NASNetLarge_model.input, outputs=output_layer)

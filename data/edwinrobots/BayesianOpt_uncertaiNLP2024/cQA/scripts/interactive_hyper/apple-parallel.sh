#!/bin/bash
#PBS -N bdl-parallel-apple
#PBS -o ./log/parallel/apple/4rounds-vanilla/5e-5-20-0.01/4rounds-mp.output
#PBS -e ./log/parallel/apple/4rounds-vanilla/5e-5-20-0.01/4rounds-mp.err
...
fake=`time python3 main.py --start_q $start_q --num_q $num_q --epochs 4 --sample_nums 20 --ilr 5e-5 --wd 0.01 --topic apple --n_iter_rounds 4 --batch_size 1 --save_dir /work/zu20361/BDL/model/ --model_name vanilla_bert_3epoch_2e-5-32-0.001 --interactive True --test_mode valid_interaction`
echo "$fake" > ./log/parallel/apple/4rounds-vanilla/5e-5-20-0.01/start-${start_q}.output

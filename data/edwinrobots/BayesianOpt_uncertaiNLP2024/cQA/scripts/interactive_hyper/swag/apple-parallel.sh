#!/bin/bash
#PBS -N apple-swag-final
#PBS -o ./log/parallel/apple/swag_hyper/1e-4-0.001/4rounds-5epochs-mp.output
#PBS -e ./log/parallel/apple/swag_hyper/1e-4-0.001/4rounds-5epochs-mp.err
#PBS -l select=1:ncpus=1:ngpus=1:mem=20G:gputype=RTX2080Ti
...
fake=`time python3 main.py --start_q $start_q --num_q $num_q --epochs 4 --sample_nums 20 --wd 0.001 --lr_init 1e-4 --topic apple --n_iter_rounds 4 --batch_size 1 --save_dir /work/zu20361/BDL/model/ --model_name swag_bert_6epoch_1e-4-16-0.001 --interactive True --test_mode valid_interaction`
echo "$fake" > ./log/parallel/apple/swag_hyper/1e-4-0.001/start-${start_q}.output

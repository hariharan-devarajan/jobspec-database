#!/bin/bash
#PBS -N bdl-travel-10sample
#PBS -o ./log/parallel/travel/30samples-dropout/4rounds-5epochs-mp.output
#PBS -e ./log/parallel/travel/30samples-dropout/4rounds-5epochs-mp.err
...
fake=`time python3 main.py --start_q $start_q --num_q $num_q --epochs 4 --sample_nums 30 --wd 0.001 --ilr 1e-4 --topic travel --n_iter_rounds 4 --batch_size 1 --save_dir /work/zu20361/BDL/model/ --model_name vanilla_bert_3epoch_2e-5-16-0.01 --interactive True`
echo "$fake" > ./log/parallel/travel/30samples-dropout/start-${start_q}.output

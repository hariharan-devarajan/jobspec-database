...
#SBATCH --job-name variant_calling_e2e_test
...
>&2 echo "+ conda activate $CONDA_PATH/bin/activate"
set +euo pipefail
conda deactivate &>/dev/null || true  # disable any existing
source $CONDA_PATH/etc/profile.d/conda.sh
conda activate  $CONDA_PATH/bin/activate # enable found
set -euo pipefail
...
snappy-snake --printshellcmds \
    ...
    --drmaa-snippet="--partition=medium" \
    --rerun-incomplete \
    -- \
    $*

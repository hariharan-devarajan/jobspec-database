...
#SBATCH --error=./results/heldout_experiment_bgnmf/errorlog-%j.err

DOCKER_IMAGE=dncb-fac-image
CONTAINER_NAME=dncb-fac-container

MLFLOW_EXPERIMENT_NAME=heldout_experiment_bgnmf
...
docker run --rm -v /data/projects/dncbtd:/work/data -v results/heldout_experiment_bgnmf:/work/data/tmp  \
    --env MLFLOW_EXPERIMENT_NAME=${MLFLOW_EXPERIMENT_NAME} ${DOCKER_IMAGE} \
    python src/dncbfac/heldout_experiment_bgnmf.py "${job_config}"
{% endraw %}
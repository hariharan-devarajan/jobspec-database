...
#SBATCH -J bertsgd                 # Job name
#SBATCH -o sbatch_logs/bertsgd.o%j # Name of stdout output file
...
mpirun_rsh --export-all -np $NODES -hostfile $HOSTFILE \
    bash run_step1.sh  --ngpus $PROC_PER_NODE --nnodes $NODES \
        --master $MASTER_RANK --output results_sgd --kfac false --resume false --mvapich

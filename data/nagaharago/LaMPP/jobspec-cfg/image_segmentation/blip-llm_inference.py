...
parser.add_argument('--use-clip-llm', default=True, metavar='DIR',
                    help='use clip and llm')
...
def clip_llm_relabel(pred_logits, pred_probs, gt_img_labels, orig_img, items, blip_output):
    uncalib_pred_labels = pred_probs.argmax(0) #basemodelのpredのprobabilityでargmaxのインデックス
    log_pred_probs = pred_probs.log() #basemodelのpredのprobのlog
    pred_items = uncalib_pred_labels.unique()    #0: wall 1: floor 21: ceiling
...
def inference():
    ...
    if args.use_clip_llm:
        processor = Blip2Processor.from_pretrained("/sqfs/work/G15445/u6b795/huggingface_ckpt/blip2-opt-2.7b_processor")
        blip = Blip2ForConditionalGeneration.from_pretrained("/sqfs/work/G15445/u6b795/huggingface_ckpt/blip2-opt-2.7b_blip")
    ...
    with torch.no_grad():
        for ex_idx, ex in pbar:
            image = ex['image']
            depth = ex['depth']
            image4blip = image.astype(np.uint8)
            ...
            # add priors
            if args.use_priors or args.use_model_chaining or args.use_clip_llm:
                ...
                if args.use_clip_llm:
                    image4blip = Image.fromarray(image4blip)
                    blip_inputs = processor(image4blip, return_tensors="pt")
                    blip_output = blip.generate(**blip_inputs)
                    pred_labels, pred_probs = clip_llm_relabel(pred[0], pred_probs, gt_img_labels=label_ds, orig_img=image_ds, items=items, blip_output=blip_output)
                    room_distr = {}
                elif args.use_model_chaining:

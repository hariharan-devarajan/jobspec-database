# cd /tmp ;   for SENS in MYD MOD ; do for DN in Day Nig ; do  qsub  -v SENS=$SENS,DN=$DN    /u/gamatull/scripts/LST/spline/sc3_fillsplineAkima_MOYD11A2_allobs.sh ; done ; done ; cd - 

#  qsub -v SENS=MYD 

# model selection http://www.nas.nasa.gov/hecc/support/kb/entry/188 
...
#PBS -V
#PBS -N sc3_fillsplineAkima_MOYD11A2_allobs.sh
...
gdalbuildvrt -te $geo_string   -separate   -overwrite  $RAMDIR/LST_${SENS}_$tile.vrt \
$( echo $(awk -v nr=$nr -v  INSENS=$INSENS -v SENS=$SENS -v DN=$DN  \'{ if (NR>nr) { print    INSENS "/LST_" SENS "_QC_day" $1 "_wgs84_" DN "_allobs_mask5noobs_Fill5obs.tif" } }\' /nobackup/gamatull/dataproces/LST/geo_file/list_day.txt ; awk -v INSENS=$INSENS -v SENS=$SENS -v  DN=$DN           \'{ print              INSENS "/LST_" SENS "_QC_day" $1 "_wgs84_" DN "_allobs_mask5noobs_Fill5obs.tif" }\' /nobackup/gamatull/dataproces/LST/geo_file/list_day.txt ; awk -v nrt=$nrt -v INSENS=$INSENS -v SENS=$SENS -v DN=$DN \'{ if (NR<=nrt) print  INSENS "/LST_" SENS "_QC_day" $1 "_wgs84_" DN "_allobs_mask5noobs_Fill5obs.tif" }\' /nobackup/gamatull/dataproces/LST/geo_file/list_day.txt)) 

gdalinfo $RAMDIR/LST_${SENS}_$tile.vrt 
...
pkfilter -of GTiff -ot Float32 -co BIGTIFF=YES -co COMPRESS=LZW -co ZLEVEL=9 -nodata 0 -f smoothnodata -dz 1 -interp akima -i $RAMDIR/LST_${SENS}_$tile.vrt -o $OUTSENS/LST_${SENS}_akima_${DN}_${tile}_tmpA.tif

cat   /proc/meminfo  | grep MemFree


rm -f $RAMDIR/LST_${SENS}_akima_$tile.vrt
gdalbuildvrt      $(seq  $(( 46-$nr+1 )) $(( 46+(46-$nr) )) |  xargs -n 1 echo -b)  $RAMDIR/LST_${SENS}_akima_$tile.vrt   $OUTSENS/LST_${SENS}_akima_${DN}_${tile}_tmpA.tif 

gdalinfo $RAMDIR/LST_${SENS}_akima_$tile.vrt 

gdal_translate  -co COMPRESS=LZW -co ZLEVEL=9 -co BIGTIFF=YES  $RAMDIR/LST_${SENS}_akima_$tile.vrt  $OUTSENS/LST_${SENS}_akima_${DN}_${tile}_allobs.tif
rm -f $OUTSENS/LST_${SENS}_akima_${DN}_${tile}_tmpA.tif  $RAMDIR/LST_${SENS}_akima_$tile.vrt 
...
qsub -v SENS=$SENS,DN=$DN  /u/gamatull/scripts/LST/spline/sc4_fillsplineAkima_merge_tile_MOYD11A2_allobs.sh
